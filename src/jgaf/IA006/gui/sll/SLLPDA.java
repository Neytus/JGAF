...
public class SLLPDA 
{
    ...
    {        
        ...
        if(top != null)
        {
            ...
            {   //je to neterminal a zhoda sedi tak orezem vstup
                buildOutputRow(top, inputWord.subList(0,1), true);
            ...
            {   // je to neterminal takze si oznacim ci nastane match
                ...
                {   // beham cez stlpce ale len tie co vyhovuju dlzke
                    ...
                    if(inputWord.size() >= prefix.size() && prefix.equals(inputWord.subList(0, prefix.size())))
                    {
                        ...
                        {
                            throw new IllegalArgumentException("ERROR: For given prefix ["+Tools.buildWord(prefix)+"] and top stack symbol ["+top+"]is no rule.\nWord cannot be generated by given gramar."); // chybna kombinacia
                        }
                    }                    
                }
    ...
    private void buildOutputRow(LLSymbol top,List<LLSymbol> rule,boolean simple)
    {
        ...
        {
            sb.append("&#09;").append(top).append("&#8594;").append(Tools.buildWord(rule)).append("</td></tr>");
        }
    }    
}
